...
function Asidebar() {
    ...
    const menuItems = [
        ...
        { href: "/dashboard/studio", icon: PlusCircle, label: "Studio", roles: ["tutor"] },
        {
        ...
        >
            <div className="flex flex-col h-full w-full">
                <div className="flex h-14 items-center border-b border-slate-800 px-6 shrink-0">
                    <Logo />
                    <div className="ml-auto block md:hidden lg:hidden xl:hidden p-1 bg-slate-800/50 text-slate-400 hover:text-black">
                        <button
                    ...
                    <nav className="grid gap-1">
                        {filteredMenuItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <item.icon className={`h-4 w-4 ${isActive ? "text-slate-800" : "text-slate-500 group-hover:text-white"}`} />
                                    {item.label}
                                </Link>
                            );
                        })}
                ...
                <div className="mt-auto border-t border-slate-800 p-4">
                    <div className="flex items-center gap-3 rounded-lg bg-slate-800/50 p-3 ring-1 ring-slate-800">
                        ...
                        <div className="flex flex-col overflow-hidden">
                            <span className="text-sm font-semibold truncate leading-none mb-1 text-slate-400">
                                {user.name}
                            </span>
                    ...
                    <button
                        // onClick={onLogout}
                        className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800/50 px-3 py-2 text-sm font-medium text-slate-600 transition-all hover:bg-red-900/20 hover:text-red-400 hover:border-red-900/30"
                    >
